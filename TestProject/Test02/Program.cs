...
namespace Test02 {
    ...
    class Program {
        ...
        private static void Exercise02(List<int> numbers) {
            var max_g = numbers.Where(s => s % 2 == 0).Max();
            Console.WriteLine(max_g);
        }
        //問題３　奇数のみを降順に並べて表示（遅延実行とする）
        //　　　　出力結果【91 87 53 35 31 17】
        private static void Exercise03(List<int> numbers) {
            var sort = numbers.Where(s => s % 2 == 1).OrderByDescending(s => s);
        ...
        private static void Exercise04(List<int> numbers) {
            var number = numbers.Where(s => s >= 10 && s <= 50);
        ...
        private static void Exercise05(List<Person> persons) {
            var age = persons.Average(s => s.Age);
            Console.WriteLine(age + "才");
        ...
        private static void Exercise06(List<Person> persons) {
            var weight = persons.Where(s => s.Height >= 170).Average(s => s.Weight);
        ...
        private static void Exercise07(List<Person> persons) {
            var humans = persons.Where(s => s.Weight <= 70).ToArray();
            foreach (var human in humans) {
                Console.WriteLine(human.Name + " " + human.Weight + "kg");
            }
        }
        ...
        private static void Exercise08(List<Person> persons) {
            var yama = persons.Where(s => s.Name.Contains("山"));
            foreach (var person in yama) {
